...
## 5. What Cannot Be Done in Explore Mode (Redirect to Act Mode)

The remaining task steps are state-changing / execution actions that Explore Mode cannot perform:

| Step | Action | Mode required |
|------|--------|---------------|
| 3 | Configure Composio MCP for this workspace | Act |
| 4 | Create/edit `.vscode/mcp.json` | Act |
| 7 | Reinstall/repair ripgrep for search_files | Act |
| 8 | Verify agent can see Composio tools after connection | Act |
| 9 | Report exact MCP config + connection status | Act (after connection) |

**Constraints honoured in this session:** no architecture changes, no production collector run, no procurement document downloads, no extra integrations added.

---

## 6. Suggested Todo List for the Act Mode Session
